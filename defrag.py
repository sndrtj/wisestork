...
def defrag(args):
    ...
    sortedList.sort()
    pdfData = []
    headerLine = ["Sample", "DEFRAG subset ChrY", "DEFRAG whole ChrY", "Determined Gender", "Total number of reads",
                  "Cluster", "% on Y"]
    ...
    for index, testSample in enumerate(sortedList):
        ...
        lines = [testSample.split('/')[-1], str((median(result) / corrMalesMedian) * 100), str(daphGender * 100),
                 str(getGender(prediction)), str(getCoverage(testSamplesPickle[testSample])), cluster,
                 str(getYPerc(testSamplesPickle[testSample], testChrom) * 100)]
        print "\t".join(lines)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--scalingFactor', type=str,
                        help='Factor that is used for correcting the calculated fetal fraction')
    ...
    parser.add_argument('outputfig', type=str, default='something',
                        help='File path (absolute preferable) of output figure (extension is added by script)')
